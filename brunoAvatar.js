...
function init() {
    ...
    world = document.getElementById('brunoavatar');
...
// #ffc107
    ...
    const head = new THREE.CylinderGeometry(140, 160, 280, 100);
    this.head = new THREE.Mesh(head, this.skinMat);
    this.head.position.x = 0;
    this.head.position.y = 150;
    this.head.position.z = 400;


    //glasses
    const glass = new THREE.CylinderGeometry(60, 28, 10, 20, 20);
    //Retinas Left
    this.glassLeft = new THREE.Mesh(glass, this.eyeMat);

    this.glassLeft.rotation.x = 80;
    ...
    //glass sidestick
    const glassSidesStick = new THREE.BoxGeometry(10, 10, 150);
    //glassLeftStick Left
    this.glassLeftStick = new THREE.Mesh(glassSidesStick, this.pupilaMat);
    this.glassLeftStick.position.x = 150;
    this.glassLeftStick.position.y = 5;
    this.glassLeftStick.position.z = 80;

    this.glassRightStick = new THREE.Mesh(glassSidesStick, this.pupilaMat);
    this.glassRightStick.position.x = -150;
    this.glassRightStick.position.y = 5;
    this.glassRightStick.position.z = 80;
    ...
    //mout
    // new THREE.CylinderGeometry(40, 40, 10, 20, 20, false, 1.5, 3.2);
    const mout = new THREE.BoxGeometry(90, 60, 50);
    this.mout = new THREE.Mesh(mout, this.skinMat);
    this.mout.position.x = 0;
    this.mout.position.z = 145;
    this.mout.position.y = -130;
    // this.mout.position.y = -130;

    //lip
    // const lip = new THREE.BoxGeometry(40, 20, 50);
    const lip = new THREE.CylinderGeometry(40, 40, 10, 10, 10, false, -1.5, 3.1);
    this.lip = new THREE.Mesh(lip, this.lipMat);
    this.lip.position.x = 0;
    this.lip.position.z = 170;
    this.lip.position.y = -110;
    this.lip.rotation.x = 1.5;
    // this.lip.position.x = 0;
    // this.lip.position.z = 162;
    // this.lip.position.y = -120;

    //hair
    const hairShap = new THREE.SphereGeometry(200, 10, 10);
    const hairMaterial = new THREE.MeshBasicMaterial({ color: '#222' });

    // const hairTop = new THREE.SphereGeometry(300, 32, 20);
    this.hair = new THREE.Mesh(hairShap, hairMaterial);
    // this.hatTop = new THREE.Mesh(hatTop, this.hatMat);
    this.hair.position.x = 0;
    this.hair.position.z = -45;
    this.hair.position.y = 180;


    const nose = new THREE.CylinderGeometry(4, 40, 100);
    this.nose = new THREE.Mesh(nose, this.skinMat);
    this.nose.position.x = 0;
    this.nose.position.z = 150;
    this.nose.position.y = -25;
    this.nose.rotation.x = -0.2;
    ...
    const materialHeart = new THREE.MeshBasicMaterial({ color: "#f00" });
    this.heart = new THREE.Mesh(geometryHeart, materialHeart);
    this.heart.position.z = 390;
    this.heart.position.x = 70;
    this.heart.position.y = 150;
    this.heart.rotation.z = 3.1;
    this.heart.scale.x = 0;
    this.heart.scale.y = 0;
    ...
    this.head.add(this.glassLeftStick);
    this.head.add(this.glassRightStick);
    ...
    this.rotate = function (speed) {
        ...
        if (isUp) {
            ...
            if (this.hair.scale.x < 2) {
                this.hair.scale.x += 0.03;
                this.hair.scale.y += 0.03;
                this.hair.scale.z += 0.03;
                this.hair.position.y += 6;
                this.hair.position.z += -2.5;

               
            }
            if (this.lip.scale.x > 0.5) this.lip.scale.x -= 0.05;

            if (this.heart.scale.x < 10) {
                this.heart.scale.x += 0.1;
                this.heart.scale.y += 0.1;
            ...
            this.armLeft.rotation.z = -1.8;
            this.armRight.rotation.z = 1.8;
            this.armLeft.position.x = -220;
            this.armRight.position.x = 220;
            this.armLeft.position.y = 125;
            this.armRight.position.y = 125;
